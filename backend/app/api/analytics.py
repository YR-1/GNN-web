...
MODEL_PERFORMANCE_SEEDS = [
    ("listsort_ageadj", "ListSort (Working Memory)", "FBNetGen", 0.3736, 0.0009, 9.7145, 1.2),
    ("pmat", "PMAT (Fluid Intelligence)", "BrainGNN", 0.3893, 0.0172, 19.6051, 2.4),
    ("picseq", "PicSeq (Picture Sequence Memory)", "FBNetGen", 0.3250, 0.0009, 8.2255, 3.1),
...
def _build_model_performance_payload() -> list[dict]:
    payload = []
    for model_id, behavioral_score, gnn_architecture, correlation, p_value, mse, seed in MODEL_PERFORMANCE_SEEDS:
        payload.append(
            {
                "id": model_id,
                "behavioralScore": behavioral_score,
                "gnnArchitecture": gnn_architecture,
